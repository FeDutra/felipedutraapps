...
async function main() {
  ...
  console.log("Inserindo requisição direta: PULSO_DIRECT_TEST_001");
  const payload = {
    ...
    origin: "lotus_live",
    mode: "text",
    input: "PULSO_DIRECT_TEST_001",
    rawInput: "PULSO_DIRECT_TEST_001\nResponda exatamente: DIRECT_OK_001",
    ...
    archived: false,
    priority: "medium",
    // adding standard openclaw payload fields
    handoff: {
        target: "openclaw",
        mode: "proposal_only"
    }
  ...
  console.log(`Aguardando processamento do motor OpenClaw... (Timeout: 30s)`);
  ...
  const unsubscribe = onSnapshot(docRef, (snap) => {
    ...
    if (data.status !== 'queued_for_openclaw' && data.status !== 'processing_by_openclaw') {
      console.log(`🎉 Resposta recebida do OpenClaw!`);
      if (data.openclawResult) {
          console.log(`Resposta Textual: ${data.openclawResult.responseText}`);
      } else {
          console.log(`Nenhum openclawResult encontrado. Dados brutos:`, JSON.stringify(data, null, 2));
      }
  ...
  timeoutId = setTimeout(() => {
    console.error("❌ TIMEOUT: O worker do OpenClaw não processou o request em 30 segundos.");
    unsubscribe();
    process.exit(1);
  }, 30000);
}
