...
function isAllowedRecordingUrl(rawUrl: string): boolean {
  try {
...
export const pulsoProcessMeeting = onRequest(
  {
    region: "us-central1",
  ...
  async (req, res) => {
    ...
    try {
      ...
      const { contextId, sessionId, chunkUrls } = req.body as {
        contextId?: string;
        sessionId?: string;
        chunkUrls?: MeetingChunkInput[];
      };
      ...
      const transcriptions: Array<{ index: number; text: string }> = [];
      const failures: Array<{ index: number; error: string }> = [];

      const transcribeChunk = async (chunk: typeof orderedChunks[number]) => {
        try {
          ...
          if (buffer.length === 0) throw new Error("empty audio chunk");
...
Remova somente hesitações sem valor semântico. Retorne exclusivamente a transcrição. Se não houver fala inteligível, retorne uma string vazia.`;
      ...
      if (!transcription) {
        await getFirestore().doc(`workspaces/felipe_dutra/pulso_meetings/${sessionId}`).set({
          status: "failed",
