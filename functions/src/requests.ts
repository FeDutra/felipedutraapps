  ...
  async (req, res) => {
    ...
    try {
          ...
          try {
            switch (type) {
              ...
              case "unlink_person_from_area": {
                ...
                let patchData: any = { updatedAt: ts };
                const reason = payload.reason || payload.notes || requestData.summary;
                if (reason) patchData.notes = reason;
                ...
                if (type === "update_person") {
                  ...
                  allowedKeys.forEach(k => {
                    if (patch[k] !== undefined) patchData[k] = patch[k];
                  });
                  ...
                  if (!projRef) return { ok: false, action: "needs_clarification", missingFields: ["projectRef"], summary: "projectRef obrigatório para vínculo." };
                  ...
                  if (!aRef) return { ok: false, action: "needs_clarification", missingFields: ["areaRef"], summary: "areaRef obrigatório para vínculo." };
              ...
              case "link_project_to_area": {
                ...
                if (type === "update_project") {
                  ...
                  if (!payload.status) return { ok: false, action: "needs_clarification", missingFields: ["status"], summary: "status obrigatório." };
                  patchData.status = payload.status;
                } else if (type === "change_project_priority") {
                  if (!payload.priority) return { ok: false, action: "needs_clarification", missingFields: ["priority"], summary: "priority obrigatório." };
                  patchData.priority = payload.priority;
                } else if (type === "link_project_to_area") {
                  if (!payload.areaRef && !requestData.areaRef) return { ok: false, action: "needs_clarification", missingFields: ["areaRef"], summary: "areaRef obrigatório." };
              ...
              case "link_task_to_area": {
                ...
                if (!taskId && payload.title) {
                  ...
                  if (!tQuery.empty) taskId = tQuery.docs[0].id;
                }
                if (!taskId) {
                ...
                if (type === "update_task") {
                  ...
                  const projRef = payload.projectRef || requestData.projectRef;
                  if (!projRef) return { ok: false, action: "needs_clarification", missingFields: ["projectRef"], summary: "projectRef obrigatório." };
                  patchData.projectRef = projRef;
                } else if (type === "link_task_to_area") {
                  const aRef = payload.areaRef || requestData.areaRef;
                  if (!aRef) return { ok: false, action: "needs_clarification", missingFields: ["areaRef"], summary: "areaRef obrigatório." };
                  patchData.areaRef = aRef;
                }
              ...
              case "unlink_source_from_area": {
                ...
                if (!sourceId && payload.name) sourceId = `source_${slugify(payload.name)}`;
                if (!sourceId) return { ok: false, action: "needs_clarification", missingFields: ["sourceRef"], summary: "sourceRef ou name obrigatório." };
                
                const sRef = db.collection(sBase).doc(sourceId);
                const sSnap = await sRef.get();
                if (!sSnap.exists) return { ok: false, action: "needs_clarification", missingFields: ["sourceRef"], summary: `Fonte não encontrada: ${sourceId}` };
      ...
      if (req.method === "POST" && path === "/progress") {
        ...
        if (!originSnap.exists) { res.status(404).send("Original request not found"); return; }
