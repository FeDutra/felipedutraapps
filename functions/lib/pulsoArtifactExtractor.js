...
exports.onPulsoRequestUpdated = exports.onPulsoRequestCreated = void 0;
...
function getAI() {
    if (!aiInstance) {
        ...
        aiInstance = new generative_ai_1.GoogleGenerativeAI(apiKey);
    }
    return aiInstance;
}
/**
 ...
 */
async function downloadFileFromStorage(storageRef) {
 ...
 */
async function processImageArtifact(buffer, mimeType, filename) {
    const ai = getAI();
    ...
    const prompt = `Analise a imagem anexa ("${filename}"). Extraia todo o texto visível (OCR) com a máxima precisão de layout. ` +
 ...
 */
async function runExtractionPipeline(requestId, data) {
    const db = admin.firestore();
    const docRef = db.collection(`workspaces/${WORKSPACE_ID}/pulso_requests`).doc(requestId);
    const attachments = data.attachments || [];
    const pendingExtraction = attachments.filter((a) => a.status === "processing_extraction");
 ...
 */
exports.onPulsoRequestCreated = (0, firestore_1.onDocumentCreated)({ document: REQUESTS_PATH, region: "us-central1", secrets: ["GEMINI_API_KEY"] }, async (event) => {
    const snap = event.data;
    if (!snap)
        return;
    await runExtractionPipeline(event.params.requestId, snap.data());
});
exports.onPulsoRequestUpdated = (0, firestore_1.onDocumentUpdated)({ document: REQUESTS_PATH, region: "us-central1", secrets: ["GEMINI_API_KEY"] }, async (event) => {
    const snap = event.data;
    if (!snap)
        return;
...
//# sourceMappingURL=pulsoArtifactExtractor.js.map