...
exports.pulsoDailyReport = exports.pulsoRegressionCheck = exports.pulsoFrontBackendSync = exports.pulsoSessionConsistency = exports.pulsoActiveMessageCheck = exports.pulsoLatencyReport = exports.pulsoQueueWatchdog = exports.pulsoHealthCheck = void 0;
...
async function ensureAlertasSession() {
    if (alertasSessionEnsured)
        return;
 ...
 */
function inspectProcessingHealth(data, now) {
...
async function resolveAlert(alertId, cronName, name) {
    const ref = db.collection(ALERTS).doc(alertId);
    const snap = await ref.get();
    if (snap.exists && snap.data()?.status === "open") {
        await ref.update({ status: "resolved", updatedAt: firestore_1.FieldValue.serverTimestamp() });
...
async function logMonitorEvent(cronName, result) {
    await db.collection(MONITOR).add({
        cron: cronName,
        runAt: firestore_1.FieldValue.serverTimestamp(),
        ...result,
    });
}
...
const LATENCY_CRITICAL_MS = 15000; // acima de 15s = vermelho
...
// ═══════════════════════════════════════════════════════════════════════════════
exports.pulsoHealthCheck = (0, scheduler_1.onSchedule)({ schedule: "every 5 minutes", region: "us-central1", timeoutSeconds: 60 }, async () => {
    ...
    const liveProcessingContexts = new Set(processingHealth.filter(item => !item.stuck && item.contextId).map(item => item.contextId));
...
// ═══════════════════════════════════════════════════════════════════════════════
exports.pulsoLatencyReport = (0, scheduler_1.onSchedule)({ schedule: "every 15 minutes", region: "us-central1", timeoutSeconds: 60 }, async () => {
...
// ═══════════════════════════════════════════════════════════════════════════════
exports.pulsoActiveMessageCheck = (0, scheduler_1.onSchedule)({ schedule: "every 10 minutes", region: "us-central1", timeoutSeconds: 60 }, async () => {
    ...
    const activeSnap = await db.collection(REQUESTS)
        ...
        .limit(50)
        .get();
    const activeMsgs = activeSnap.docs.map(d => ({ id: d.id, ...d.data() }));
...
// ═══════════════════════════════════════════════════════════════════════════════
exports.pulsoSessionConsistency = (0, scheduler_1.onSchedule)({ schedule: "every 30 minutes", region: "us-central1", timeoutSeconds: 60 }, async () => {
    ...
    for (const doc of snap.docs) {
        ...
        if (!d.contextId)
            missingContext++;
        if (!d.openclawSessionKey)
            missingSessionKey++;
        if (d.contextId)
            contextIds.add(d.contextId);
    }
...
exports.pulsoRegressionCheck = (0, scheduler_1.onSchedule)({ schedule: "every 60 minutes", region: "us-central1", timeoutSeconds: 90 }, async () => {
    ...
    for (const doc of snap.docs) {
        ...
        // Resposta vazia
        if (d.status === "success" && !responseText && activeActive)
            emptyResponse++;
        // Status incoerente (success mas com errors)
        if (d.status === "success" && d.openclawResult?.errors?.length > 0)
            inconsistentStatus++;
        // Success sem efeito: não tem resposta inline nem active_message
        if (d.status === "success" && !responseText)
            successNoEffect++;
    }
...
async function ensureRelatoriosSession() {
...
// ═══════════════════════════════════════════════════════════════════════════════
exports.pulsoDailyReport = (0, scheduler_1.onSchedule)({
    schedule: "15 9 * * *",
...
//# sourceMappingURL=pulsoMonitor.js.map