  ...
  updateActiveVoices(
    ...
    somNodes.forEach(node => {
      ...
      if (voice.audioSource) {
        ...
        const pitchMultiplier = ((baseFreq + jitterVal) / 220) * nodePitch; // 220 is standard A3 base
        ...
        for (let pi = currentCount; pi < targetCount; pi++) {
          ...
          try {
            ...
            const freq = baseFreq * this.globalPitchMultipliers[pi] * nodePitch;
        ...
        voice.polyOscPairs.forEach(([oscA, oscB], pi) => {
          const freq = (baseFreq + jitterVal) * (this.globalPitchMultipliers[pi] ?? 1.0) * nodePitch;
          if (isFinite(freq)) {
